...
static int lcore_rx_and_distributor(struct lcore_params *p) {
  ...
  while (!quit_signal_rx) {
    ...
    uint16_t sent = rte_ring_enqueue_burst(tx_ring, (void *)bufs, nb_ret, NULL);
...
static int lcore_tx(struct rte_ring *in_r) {
  ...
  while (!quit_signal) {
    ...
    RTE_ETH_FOREACH_DEV(port) {
      ...
      const uint16_t nb_rx =
          rte_ring_dequeue_burst(in_r, (void *)bufs, BURST_SIZE_TX, NULL);
...
int main(int argc, char *argv[]) {
  ...
  if (enable_lcore_rx_distributor) {
    /* RX and distributor combined, 3 fixed function cores (stat, TX, at least 1
     * worker) */
    min_cores = 4;
    num_workers = rte_lcore_count() - 3;
  } else {
    /* separate RX and distributor, 3 fixed function cores (stat, TX, at least 1
     * worker) */
    min_cores = 5;
  ...
  rx_dist_ring =
      rte_ring_create("Input_ring", SCHED_RX_RING_SZ, rte_socket_id(),
                      RING_F_SC_DEQ | RING_F_SP_ENQ);
  ...
  /* Start tx core */
  rte_eal_remote_launch((lcore_function_t *)lcore_tx, dist_tx_ring, tx_core_id);
  ...
  if (!pr)
    rte_panic("malloc failure\n");
  *pr = (struct lcore_params){worker_id++, d, rx_dist_ring, dist_tx_ring,
                              mbuf_pool};
  if (enable_lcore_rx_distributor)
    rte_eal_remote_launch((lcore_function_t *)lcore_rx_and_distributor, pr,
                          rx_core_id);
  else
    rte_eal_remote_launch((lcore_function_t *)lcore_rx, pr, rx_core_id);
